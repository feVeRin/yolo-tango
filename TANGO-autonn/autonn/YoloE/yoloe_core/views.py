...
def start(request):
    ...
    pr = multiprocessing.Process(
        target=process_yolo, args=(userid, project_id, data_yaml, proj_yaml)
    )
...
def get_user_requirements(userid, projid):
    ...
    if os.path.isdir("/shared/datasets/" + dataset_on_proj):
        ...
        print(
            f"There is no /shared/datasets/{dataset_on_proj}. Instead COCO dataset will be used."
        )
...
def process_yolo(userid, project_id, data_yaml, proj_yaml):
    try:
        ...
        final_model = run_yolo(
            proj_path, str(data_yaml), target=target_info, train_mode="search"
        )
        ...
        create_nn_info(
            src_info_path, final_info_path, best_pt_path, input_shape, target_device
        )
        ...
        shutil.copy(
            proj_yaml,
            Path(proj_path) / str("exp" + str(exp_num) + "_project_info.yaml"),
        )
...
def create_nn_info(
    ...
    final_py_list = str(
        "['models/yolo.py', 'basemodel.yaml', 'models/common.py', 'models/experimental.py', 'utils/autoanchor.py', 'utils/datasets.py', 'utils/general.py', 'utils/torch_utils.py', 'utils/loss.py', 'utils/metrics.py', 'utils/plots.py']"
    )
    nn_info = dict()
...
def get_ready_for_test(request):
    try:
        ...
        data_yaml["train"] = str(
            Path("/shared/") / "datasets" / "coco" / "images" / "train2017"
        )
        data_yaml["test"] = str(
            Path("/shared/") / "datasets" / "coco" / "images" / "train2017"
        )
