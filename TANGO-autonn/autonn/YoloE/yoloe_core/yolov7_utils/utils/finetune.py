...
def finetune(subnet, hyp, opt, device, tb_writer=None):
    ...
    if cuda and rank != -1:
        subnet = DDP(
            ...
            find_unused_parameters=any(
                isinstance(layer, nn.MultiheadAttention) for layer in subnet.modules()
            ),
        )
    ...
    for epoch in range(
        ...
        if opt.image_weights:
            ...
            if rank in [-1, 0]:
                cw = (
                    subnet.class_weights.cpu().numpy() * (1 - maps) ** 2 / nc
                )  # class weights
        ...
        if rank in [-1, 0]:
            ...
            if best_fitness == fi:
                ckpt = {
                    ...
                    "model": deepcopy(
                        subnet.module if is_parallel(subnet) else subnet
                    ).half(),
