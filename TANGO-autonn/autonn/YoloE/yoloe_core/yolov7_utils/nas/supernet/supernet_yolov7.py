...
## search_block.py
from .search_block import ELAN
from .search_block import ELANBlock
...
class YOLOSuperNet(YOLOModel):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if isinstance(m, ELAN) and isinstance(
                self.runtime_depth, list
            ):  # subnetwork run
        ...
        if preserve_weight:
            ...
            for i, m in enumerate(model):
                if isinstance(m, ELAN):
                    ...
                    model[i] = ELANBlock(
                        m.mode, deepcopy(m.layers[: act_idx + 1]), depth
                    )
                    model[i].i, model[i].f, model[i].type, model[i].np = (
                        m.i,
                        m.f,
                        m.type,
                        m.np,
