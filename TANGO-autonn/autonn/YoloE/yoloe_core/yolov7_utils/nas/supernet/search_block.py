...
from nas.supernet.dynamic_layers.dynamic_op import DynamicBatchNorm2d
from nas.supernet.dynamic_layers.dynamic_op import DynamicConv2d
...
class ELANBlock(nn.Module):
    def __init__(self, mode, layers, depth):
        ...
        if mode == "BBone":
            self.act_idx = [
                idx for idx in range(depth * 2) if (idx % 2 == 1 or idx == 0)
            ]  # it is only for Bbone
        elif mode == "Head":
...
class TinyELAN(ELAN2):
    ...
    def __init__(
        self, c1, c2, k, depth, act
    ):  # activation default : SILU() / inputs could be : nn.ReLU(), nn.LeakyReLU() ...
