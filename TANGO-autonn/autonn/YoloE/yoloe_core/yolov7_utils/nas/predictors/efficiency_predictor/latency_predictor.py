...
class LatencyPredictor:
    def __init__(self, target, target_acc, device):
        ...
        bpred_path = os.path.join(
            path, "trained_lat_pred/{}_{}_backbone.pt".format(target, target_acc)
        )
        hpred_path = os.path.join(
            path, "trained_lat_pred/{}_{}_head.pt".format(target, target_acc)
        )

        self.device = device
        self.b_net = Net(
            nfeat=20, hw_embed_on=False, hw_embed_dim=0, layer_size=64
        ).cuda()
        self.h_net = Net(
            nfeat=28, hw_embed_on=False, hw_embed_dim=0, layer_size=64
        ).cuda()
...
def arch_to_feat(arch, device):
    ...
    return (
        torch.Tensor(b_onehot).to(device).float(),
        torch.Tensor(h_onehot).to(device).float(),
    )
