...
from .syolo_utils.general import ap_per_class
from .syolo_utils.general import box_iou
from .syolo_utils.general import check_file
from .syolo_utils.general import check_img_size
from .syolo_utils.general import clip_coords
from .syolo_utils.general import coco80_to_coco91_class
from .syolo_utils.general import compute_loss
from .syolo_utils.general import non_max_suppression
from .syolo_utils.general import output_to_target
from .syolo_utils.general import plot_images
from .syolo_utils.general import scale_coords
from .syolo_utils.general import xywh2xyxy
from .syolo_utils.general import xyxy2xywh
from .syolo_utils.torch_utils import select_device
from .syolo_utils.torch_utils import time_synchronized
...
def test(data, weights=None, batch_size=16, imgsz=640, conf_thres=0.001,
         ...
         verbose=False, model=None, dataloader=None, save_dir='',
    ...
    if not training:
        ...
        dataloader = create_dataloader(path, imgsz, batch_size,
                                       model.stride.max(), opt,
                                       hyp=None, augment=False,
                                       cache=False, pad=0.5, rect=True)[0]
    ...
    for batch_i, (img, targets, paths, shapes) \
        ...
        with torch.no_grad():
            ...
            # inf_out, train_out = \
            #     model(img.to(memory_format=torch.channels_last),
            #           augment=augment)
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                pred[:, :4] = \
                    scale_coords(img[si].shape[1:], pred[:, :4],
                                 shapes[si][0], shapes[si][1])  # to original
            ...
            stats.append((correct.cpu(), pred[:, 4].cpu(),
                          pred[:, 5].cpu(), tcls))
    ...
    if not training:
        print('Speed: %.1f/%.1f/%.1f ms inference/NMS/total per \
              %gx%g image at batch-size %g' % t)
    ...
    if save_json and len(jdict):
        ...
        try:
            ...
            val_path = '/shared/datasets/coco/annotations/instances_val*.json'
            cocoGt = COCO(glob.glob(val_path)[0])
