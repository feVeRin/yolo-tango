...
OPS = {
    # '3x3_BNCSP2': lambda c1, c2, n:
    # MBottleneckCSP2Layer(c1, c2, n, 3, activation='none'),
    '3x3_relu_BNCSP2': lambda c1, c2, n:
    MBottleneckCSP2Layer(c1, c2, n, 3, activation='relu'),
    '3x3_leaky_BNCSP2': lambda c1, c2, n:
    MBottleneckCSP2Layer(c1, c2, n, 3, activation='leaky'),
    '3x3_mish_BNCSP2': lambda c1, c2, n:
    MBottleneckCSP2Layer(c1, c2, n, 3, activation='mish'),
    # '5x5_BNCSP2': lambda c1, c2, n:
    # MBottleneckCSP2Layer(c1, c2, n, 5, activation='none'),
    '5x5_relu_BNCSP2': lambda c1, c2, n:
    MBottleneckCSP2Layer(c1, c2, n, 5, activation='relu'),
    '5x5_leaky_BNCSP2': lambda c1, c2, n:
    MBottleneckCSP2Layer(c1, c2, n, 5, activation='leaky'),
    '5x5_mish_BNCSP2': lambda c1, c2, n:
    MBottleneckCSP2Layer(c1, c2, n, 5, activation='mish')
}
...
class ActConv(nn.Module):
    ...
    # ch_in, ch_out, kernel, stride, padding, groups, activation function
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, activation='mish'):
...
class MBottleneck(nn.Module):
    ...
    # ch_in, ch_out, shortcut, groups, expansion, activation function
    def __init__(self, c1, c2, kernel_size=3, shortcut=True, g=1,
                 e=0.5, activation='mish'):
        super(MBottleneck, self).__init__()
        c_ = int(c2 * e)  # hidden channels

        self.cv1 = ActConv(c1, c_, 1, 1, activation=activation)
        self.cv2 = ActConv(c_, c2, k=kernel_size, s=1, g=g,
...
class MBottleneckCSP2Layer(nn.Module):
    ...
    # ch_in, ch_out, number, shortcut, groups, expansion, activation function
    def __init__(self, c1, c2, n=1, kernel_size=3, shortcut=False, g=1,
                 e=0.5, activation='mish'):
        ...
        self.m = nn.Sequential(*[MBottleneck(c_, c_, kernel_size=kernel_size,
                                             shortcut=shortcut, g=g, e=1.0,
                                             activation=activation)
                                 for _ in range(n)])
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        return \
            self.conv(torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2],
                                 x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1))


# class Upsample(nn.Module):
#     # Upsamples a given data
#     # size, scale_factor, mode, align_corner, recompute_scale_factor
#     def __init__(self, s=None, s_f=None, m='nearest', a_c=None):
#         super(Upsample, self).__init__()
#         self.Upsample = nn.Upsample(s, s_f, m, a_c)
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p),
                              groups=g, bias=False)  # to x(b,c2,1,1)
        self.flat = Flatten()

    def forward(self, x):
        z = torch.cat([self.aap(y) for y in (x if isinstance(x, list)
                                             else [x])], 1)  # cat if list
...
class DWConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels,  stride=1,  bias=False):
        ...
        groups = in_channels
        # kernel = 3
        # print(kernel, 'x', kernel, 'x', out_channels, 'x',
...
class HarDBlock(nn.Module):
    ...
    def __init__(self, in_channels, growth_rate, grmul, n_layers,
        ...
        for i in range(n_layers):
            outch, inch, link = \
                self.get_link(i+1, in_channels, growth_rate, grmul)
...
class HarDBlock2(nn.Module):
    ...
    def __init__(self, in_channels, growth_rate, grmul, n_layers,
        ...
        for i in range(n_layers):
            ...
            conv_layers_.append(nn.Conv2d(cur_ch, accum_out_ch, kernel_size=3,
        ...
        for i in range(len(self.conv_layers)):
            ...
            if b_src is not None:
                if trt:
                    ...
                    # for pytorch, add bias with standalone tensor
                    # is more efficient than within conv.bias
                    # this is because the amount of non-zero bias is small,
                    # but if we use conv.bias,
                    # the number of bias will be much larger
            ...
            # update BatchNorm or remove it
            # if there is no BatchNorm in the v1 block
