...
from .general import torch_distributed_zero_first
...
def create_dataloader(path, imgsz, batch_size, stride, single_cls, hyp=None,
    ...
    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1
             else 0, 8])  # number of workers
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640):
        ...
        images = [x for x in files if os.path.splitext(x)[-1].lower()
                  in img_formats]
        videos = [x for x in files if os.path.splitext(x)[-1].lower()
                  in vid_formats]
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            print('video %g/%g (%g/%g) %s: ' %
                  (self.count + 1, self.nf, self.frame,
        ...
        # cv2.imwrite(path + '.letterbox.jpg',
        #             255 * img.transpose((1, 2, 0))[:, :, ::-1])
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False,
                 hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size /
                                        stride + pad).astype(np.int) * stride
        ...
        for i, file in enumerate(pbar):
            lbl = self.labels[i]  # label
            if lbl.shape[0]:
                assert lbl.shape[1] == 5, '> 5 label columns: %s' % file
                assert (lbl >= 0).all(), 'negative labels: %s' % file
                assert (lbl[:, 1:] <= 1).all(), \
                    'non-normalized or out of bounds coordinate labels: %s' \
                    % file
                if np.unique(lbl, axis=0).shape[0] < lbl.shape[0]:
                    # duplicate rows
                    nd += 1
                    # print('WARNING: duplicate rows in %s' %
                    #       self.label_files[i])
                if single_cls:
                    lbl[:, 0] = 0  # force dataset into single-class mode
                self.labels[i] = lbl
                nf += 1  # file found
                ...
                if create_datasubset and ns < 1E4:
                    ...
                    exclude_classes = 43
                    if exclude_classes not in lbl[:, 0]:
                        ns += 1
        ...
        if cache_images:
            ...
            for i in pbar:  # max 10k images
                self.imgs[i], self.img_hw0[i], self.img_hw[i] = \
                    load_image(self, i)  # img, hw_original, hw_resized
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            if random.random() < hyp['mixup']:
                img2, labels2 = \
                    load_mosaic(self, random.randint(0, len(self.labels) - 1))
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        # always resize down,
        # only resize up if training with augmentation
        if r != 1:
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            x1b, y1b, x2b, y2b = \
                w - (x2a - x1a), 0, max(xc, w), min(y2a - y1a, h)
    ...
    img4, labels4 = \
        random_perspective(img4, labels4,
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1,
    ...
    if n:
        ...
        # xy = np.concatenate((x - w / 2, y - h / 2, x + w / 2,
        #                      y + h / 2)).reshape(4, n).T
...
def reduce_img_size(path='path/images', img_size=1024):
    # creates a new ./images_reduced folder
    # with reduced size images of maximum size img_size
