...
from yolov5_utils.general import check_img_size
from yolov5_utils.general import check_yaml
from yolov5_utils.general import coco80_to_coco91_class
from yolov5_utils.general import colorstr
from yolov5_utils.general import increment_path
from yolov5_utils.general import non_max_suppression
from yolov5_utils.general import print_args
from yolov5_utils.general import scale_coords
from yolov5_utils.general import xywh2xyxy
from yolov5_utils.general import xyxy2xywh
from yolov5_utils.metrics import ConfusionMatrix
from yolov5_utils.metrics import ap_per_class
from yolov5_utils.metrics import box_iou
from yolov5_utils.plots import output_to_target
from yolov5_utils.plots import plot_images
from yolov5_utils.plots import plot_val_study
from yolov5_utils.torch_utils import select_device
from yolov5_utils.torch_utils import time_sync
...
def run(
    ...
    assert data is None or dataloader is None, \
        f'data: {data}, dataloader: {dataloader}'
    ...
    if training:  # called by train.py
        ...
        device, pt, jit, engine = \
            next(model.parameters()).device, True, False, False
        half &= device.type != 'cpu'  # half precision only supported on CUDA
        model.half() if half else model.float()
        nc = 1 if single_cls else int(model.nc)  # number of classes
    else:  # called directly (tenace comment: DO NOT NEED IT)
        pass
        ...
        #     model.stride, model.pt, model.jit, model.engine
        ...
        #                     f' (1,3,{imgsz},{imgsz}) for non-PyTorch models')
        # # Data
    ...
    if dataloader is None:
        # COCO dataset
        is_coco = isinstance(data.get('val'), str) \
            and data['val'].endswith(f'coco{os.sep}val2017.txt')
    else:
        is_coco = False
    ...
    # Dataloader (tenace comment: DO NOT NEED IT)
    # if not training:  # called directly
    #     if pt and not single_cls:  # check --weights are trained on --data
    #         ncm = model.model.nc
    #         assert ncm == nc, \
    #             f'{weights[0]} ({ncm} classes) trained on different --data'\
    #             f' than what you passed ({nc} '\
    #             f'classes). Pass correct combination of --weights'\
    #             f' and --data that are trained together.'
    #     # warmup
    ...
    names = {k: v for k, v in enumerate(model.names
             if hasattr(model, 'names') else model.module.names)}
    ...
    # progress bar
    pbar = \
        tqdm(dataloader, desc=s, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        out, train_out = model(im) \
            if training else model(im, augment=augment, val=True)
        dt[1] += time_sync() - t2
    ...
    if len(stats) and stats[0].any():
        tp, fp, p, r, f1, ap, ap_class = \
            ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)
        ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
        mp, mr, map50, map = \
            p.mean(), r.mean(), ap50.mean(), ap.mean()
    ...
    if not training:
        ...
        LOGGER.info(f'Speed: %.1fms pre-process,'
                    f' %.1fms inference, %.1fms NMS per image'
                    f' at shape {shape}' % t)
    ...
    if save_json and len(jdict):
        if data is None:
            raise NameError('\'data\' must be defined to save json')
        ...
        try:
            ...
            if is_coco:
                ...
                eval.params.imgIds = [int(Path(x).stem) for x
                                      in dataloader.dataset.im_files]
            eval.evaluate()
            eval.accumulate()
            eval.summarize()
...
def parse_opt():
    ...
    parser.add_argument('--task',
                        default='val', help='train, val, test, speed or study')
    ...
    parser.add_argument('--verbose',
                        action='store_true', help='report mAP by class')
    ...
    parser.add_argument('--project',
                        default=ROOT / 'runs/val', help='save to project/name')
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        if opt.conf_thres > 0.001:
            ...
            LOGGER.info(f'WARNING: '
                        f'confidence threshold {opt.conf_thres} >> 0.001 '
                        f'will produce invalid mAP values.')
        run(**vars(opt))

    else:
        weights = opt.weights \
            if isinstance(opt.weights, list) else [opt.weights]
        opt.half = True  # FP16 for fastest results
        if opt.task == 'speed':  # speed benchmarks
            # python val.py --task speed --data coco.yaml --batch 1
            #               --weights yolov5n.pt yolov5s.pt...
