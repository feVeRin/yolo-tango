...
from .general import CONFIG_DIR
from .general import FONT
from .general import LOGGER
from .general import Timeout
from .general import check_font
from .general import clip_coords
from .general import increment_path
from .general import is_ascii
from .general import try_except
...
def check_pil_font(font=FONT, size=10):
    ...
    try:
        return ImageFont.truetype(str(font)
                                  if font.exists() else font.name, size)
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf',
        ...
        if self.pil:  # use PIL
            self.im = im \
                if isinstance(im, Image.Image) else Image.fromarray(im)
...
def hist2d(x, y, n=100):
    ...
    xedges, yedges = np.linspace(x.min(), x.max(), n), \
...
def output_to_target(output):
    ...
    for i, o in enumerate(output):
        for *box, conf, cls in o.cpu().numpy():
            targets.append([i, cls,
                           *list(*xyxy2xywh(np.array(box)[None])), conf])
...
def plot_val_txt():
    # from yolov5_utils.plots import *; plot_val()
...
def plot_targets_txt():
    # from yolov5_utils.plots import *; plot_targets_txt()
...
def plot_val_study(file='', dir='', x=None):
    ...
    # for f in [save_dir / f'study_coco_{x}.txt' for x
    #     in ['yolov5n6', 'yolov5s6', 'yolov5m6', 'yolov5l6', 'yolov5x6']]:
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    plt.savefig(save_dir / 'label_correlogram.jpg', dpi=200)
    plt.close()
...
def plot_evolve(evolve_csv='path/to/evolve.csv'):
    ...
    for i, k in enumerate(keys[7:]):
        ...
        plt.subplot(6, 5, i + 1)
        plt.scatter(v, f, c=hist2d(v, f, 20), cmap='viridis', alpha=.8,
                    edgecolors='none')
...
def plot_results(file='path/to/results.csv', dir=''):
    ...
    # Usage:
    #   from yolov5_utils.plots import *
...
def save_one_box(xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10,
                 square=False, BGR=False, save=True):
    # Save image crop as {file}
    #   with crop size multiple {gain} and {pad} pixels.
    # Save and/or return crop
    ...
    if save:
        ...
        # https://github.com/ultralytics/yolov5/issues/7007
        # cv2.imwrite(f, crop)
        Image.fromarray(cv2.cvtColor(
            crop,
            cv2.COLOR_BGR2RGB)).save(f, quality=95, subsampling=0)
    return crop
