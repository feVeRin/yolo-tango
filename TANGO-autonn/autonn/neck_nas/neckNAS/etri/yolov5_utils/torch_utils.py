...
from .general import file_update_date
from .general import git_describe
...
def torch_distributed_zero_first(local_rank: int):
    """ Decorator to make all processes in distributed training wait
...
def device_count():
    ...
    # Safe version of torch.cuda.device_count(). Only works on Linux.
...
def select_device(device='', batch_size=0, newline=True):
    ...
    if cpu:
        ...
        assert torch.cuda.is_available() \
               ...
               "Invalid CUDA '--device %s' requested," \
               " use '--device cpu' or pass valid CUDA device(s)" \
               % (device)
    ...
    if not newline:
        s = s.rstrip()
    LOGGER.info(
        s.encode().decode('ascii', 'ignore') if platform.system() == 'Windows'
        else s)  # emoji-safe
...
def profile(input, ops, n=10, device=None):
    ...
    print(f"{'Params':>12s}{'GFLOPs':>12s}{'GPU_mem (GB)':>14s}"
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                for _ in range(n):
                    ...
                    t[1] = time_sync()
                    try:
                        _ = ((sum(yi.sum() for yi in y) if isinstance(y, list)
                             else y).sum().backward())
                        t[2] = time_sync()
                ...
                s_in = tuple(x.shape) \
                    if isinstance(x, torch.Tensor) else 'list'
                s_out = tuple(y.shape) \
                    if isinstance(y, torch.Tensor) else 'list'
                p = sum(list(x.numel() for x in m.parameters())) \
                    if isinstance(m, nn.Module) else 0  # parameters
                print(f'{p:12}{flops:12.4g}{mem:>14.3f}{tf:14.4g}{tb:14.4g}'
...
def initialize_weights(model):
    for m in model.modules():
        ...
        if t is nn.Conv2d:
            ...
            m.eps = 1e-3
            m.momentum = 0.03
        elif t in [nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.Mish,
                   nn.SiLU]:
            m.inplace = True
...
def fuse_conv_and_bn(conv, bn):
    ...
    _mul = bn.running_mean
    _div = bn.running_var + bn.eps
    b_bn = bn.bias - bn.weight.mul(_mul).div(torch.sqrt(_div))
...
def model_info(model, verbose=False, img_size=640):
    ...
    if verbose:
        print(f"{'layer':>5} {'name':>40} {'gradient':>9} {'parameters':>12}"
    ...
    try:  # FLOPs
        ...
        stride = max(int(model.stride.max()), 32) \
            if hasattr(model, 'stride') else 32
        img = torch.zeros(
        ...
        # expand if int/float
        img_size = img_size \
            if isinstance(img_size, list) else [img_size, img_size]
        # 640x640 GFLOPs
    ...
    name = Path(model.yaml_file).stem.replace('yolov5', 'YOLOv5') \
        if hasattr(model, 'yaml_file') else 'Model'
    LOGGER.info(f"{name} summary: {len(list(model.modules()))} layers,"
                f" {n_p} parameters, {n_g} gradients{fs}")
...
class EarlyStopping:
    ...
    def __call__(self, epoch, fitness):
        ...
        if stop:
            LOGGER.info(f'Stopping training early as no improvement observed'
                        f' in last {self.patience} epochs. '
                        f'Best results observed at epoch {self.best_epoch},'
                        f' best model saved as best.pt.\n'
                        f'To update EarlyStopping(patience={self.patience})'
                        f' pass a new patience value, '
                        f'i.e. `python train.py --patience 300`'
                        f' or use `--patience 0` to disable EarlyStopping.')
        return stop
...
class ModelEMA:
    ...
    def update_attr(self, model,
                    include=(), exclude=('process_group', 'reducer')):
