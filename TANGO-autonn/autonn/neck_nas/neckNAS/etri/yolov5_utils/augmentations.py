...
from yolov5_utils.general import check_version
from yolov5_utils.general import colorstr
from yolov5_utils.general import resample_segments
from yolov5_utils.general import segment2box
...
class Albumentations:
    ...
    def __init__(self):
        ...
        try:
            ...
            LOGGER.info(colorstr('albumentations: ')
                        + ', '.join(f'{x}' for x
                        in self.transform.transforms if x.p))
    ...
    def __call__(self, im, labels, p=1.0):
        if self.transform and random.random() < p:
            ...
            im, labels = \
                new['image'], \
                np.array([[c, *b] for c, b
                         in zip(new['class_labels'], new['bboxes'])])
        return im, labels
...
def replicate(im, labels):
    ...
    for i in s.argsort()[:round(s.size * 0.5)]:  # smallest indices
        ...
        labels = np.append(labels,
                           [[labels[i, 0], x1a, y1a, x2a, y2a]], axis=0)
...
def random_perspective(im,
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():
        if perspective:
            im = cv2.warpPerspective(im, M, dsize=(width, height),
                                     borderValue=(114, 114, 114))
        else:  # affine
            im = cv2.warpAffine(im, M[:2], dsize=(width, height),
                                borderValue=(114, 114, 114))
